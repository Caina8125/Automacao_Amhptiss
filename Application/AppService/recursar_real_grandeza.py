...
class RecursoReal(PageElement):
    ...
    recurso_de_glosa_menu = (By.XPATH, '//*[@id="menuPrincipal"]/div/div[8]/a')
        ...
        while count < 10:
            try:
                ...
                for planilha in lista_de_planilhas:
                    ...
                    lote = f"{df['Protocolo Glosa'][0]}".replace(".0", "")
                    self.driver.find_element(*self.codigo).send_keys(lote)
                    time.sleep(2)
                    ...
                    if 'A fatura não possui itens para gerar o lote de recurso de glosa ou já existem lotes gerados para a mesma.' in content:
                        ...
                        time.sleep(2)
                        self.driver.find_element(*self.fatura_input).send_keys(lote)
                        time.sleep(2)
...
def recursar_real(user, password):
    try:
        global driver, url
        ...
        login_page.exe_login(usuario, senha)
        Caminho(driver, url).exe_caminho()
        RecursoReal(driver, url).fazer_recurso(pasta)
        driver.quit()
